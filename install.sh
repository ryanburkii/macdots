...
usage() {
  echo "Usage: $0 [--full | --config]"
  echo ""
  echo "  --full    Install Homebrew, packages, configs, and start services"
  echo "  --config  Link configs to ~/.config only (no packages or services)"
  exit 1
}
...
case "${1:-}" in
  ...
  "")
    ...
    echo "  1) full    — Homebrew, packages, configs, services"
    echo "  2) config  — configs only (link to ~/.config)"
    echo ""
    read -rp "Choice [1/2]: " choice
    case "$choice" in
      1|full)   MODE="full" ;;
      2|config) MODE="config" ;;
      *) warn "Invalid choice."; usage ;;
    esac
    ;;
  *) usage ;;
esac

# ── Config install (shared) ───────────────────────────────────────────────────
install_config() {
